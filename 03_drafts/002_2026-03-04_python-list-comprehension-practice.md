...
**STONE**: AG先生、最近他人のPythonコードを読むと、リストの括弧 `[]` の中に長い数式や `for` とか `if` がぎっしり詰まった暗号みたいな行をよく見るんです。あれ、何ですか？

**AG**: ストーンさん、それは「内包表記（Comprehension）」ですね！ Pythonの特徴的な文法の一つで、リストや辞書をたった1行でスッキリ作れる魔法の構文なんです。

**STONE**: ええっ、1行で！？ 僕はいつも空のリストを作ってから `for` で回して `append()` してるんですけど……。

**AG**: それでも間違いではありませんよ。でも、内包表記をマスターすれば、コードの行数が減ってバグも減り、さらに実行速度まで速くなることが多いんです。今日はその暗号を解読して、実戦で使えるように「10本ノック」に挑戦してみましょう！

[Image Plan]
[Gen-AI Prompt]: A bright conceptual illustration. A cute shrimp character named Stone is looking confused at a complicated, tangled multi-line machine. Next to him, a friendly robot teacher named AG presents a sleek, simple, single-box glowing machine labeled "Comprehension".
[Filename]: 002_python-list-comprehension-practice_01.png
[AltText]: 複雑なfor文からシンプルな内包表記へ

## 1. 基礎編：リスト、辞書、集合の内包表記の基本構文

**AG**: 内包表記の基本のカタチは `[式 for 変数 in イテラブル]` です。実はリストだけでなく、カッコの形を変えれば辞書（`{}`）や集合（`{}`）も作れるんですよ。

[Code: Insert list_comprehension.py Part 1]
```python
# === 1. 基礎編：リスト、辞書、集合の内包表記の基本構文 ===
...
**AG**: リストなら `[]`、辞書なら `{キー: 値 for ...}`、集合なら `{式 for ...}` と書くだけなんです。
**STONE**: わぁ、ホントだ！カッコを変えるだけで辞書まで作れちゃうんですね！

## 2. パラダイムシフト編：for.append()からの書き換え

[Conversation: STONE's Question]
**STONE**: でも先生、やっぱり `for` と `if` が混ざるとどう読んで良いか分からなくなります。

**AG**: じゃあ、見慣れた `append()` のコードと見比べて、「どこがどう移動したか」を下から上へパズルみたいに当てはめてみましょう。

[Code: Insert list_comprehension.py Part 2]
```python
# === 2. パラダイムシフト編：for.append()からの書き換え ===
...
[AG's Explanation: Walkthrough]
**AG**: まず一番左の `num` が「最終的に追加したい式（`append()`の中身）」です。その次に「ループ（`for`）」、最後に「条件（`if`）」が続きます。
**STONE**: なるほど！「何を」「どう回して」「どんな条件で」の順番に並んでいるんですね。そうやって分解して読めば怖くないかも。

## 3. アンチパターン編：過度なネストと可読性

[Conversation: STONE's Question]
**STONE**: 面白くなってきました！ じゃあ、for文を3重くらいネストさせて、if文もたくさん繋げて、超絶アクロバティックな1行コードにしちゃってもいいんですか！？

**AG**: ストーンさん、落ち着いて！（笑） それが一番やってはいけない「アンチパターン」です。

[Code: Insert list_comprehension.py Part 3]
```python
# === 3. アンチパターン編：過度なネストと可読性 ===
...
[AG's Explanation: Walkthrough]
**AG**: 何でもかんでも1行に詰め込めば良いというものではありません。`for` や `if` が複数重なるような複雑な処理は、可読性が著しく低下します。無理に1行にせず、カッコ内で適切に改行を入れてあげるのが「良いコード」の秘訣ですよ。

[Image Plan]
[Gen-AI Prompt]: A humorous 3D illustration. A cute shrimp character named Stone is tangled in a giant knot of wires representing "nested loops". A robot teacher named AG is cutting through the knot with glowing safety scissors, leaving a clean, straight wire (list comprehension).
[Filename]: 002_python-list-comprehension-practice_02.png
[AltText]: 複雑なネストを適切に整理するAG先生

## 4. 実践編：よく使う10本ノック

[Conversation: STONE's Question]
**STONE**: 分かりました！ 可読性を意識しながら、実戦でどう使うのかもっと教えてください。

**AG**: もちろんです。実務や競技プログラミングなどですぐに使える具体的なパターンを10個連続でお見せしますよ！

[Code: Insert list_comprehension.py Part 4]
```python
# === 4. 実践編：よく使う10本ノック ===
...
[AG's Explanation: Walkthrough]
**AG**: 文字列のフィルタリングから型変換、辞書のキー・バリュー入れ替えなど、内包表記はデータの変形に大活躍します。さらに、前回学んだ `pathlib` と組み合わせれば、「特定フォルダ内の `.py` ファイル名だけをリストアップする」なんてこともたった1行で書けちゃうんです！
**STONE**: （拍手）エビのハサミでもこれならスラスラ入力できそう🦐！ 10本ノックで完全にコツを掴みました。ありがとうございます、AG先生！

---

[Conversation: STONE's Question]
**AG**: ストーンさん、内包表記の見やすさに感動した方に、ぜひおすすめしたい一冊があるんです。
**STONE**: わあ、なんですか！？
**AG**: **『Effective Python 第2版――Pythonプログラムを改良する90項目』** です。
**STONE**: イフェクティブ……響きからして強そうですね🦐
**AG**: 今回紹介した「内包表記は複雑にしすぎない」といったベストプラクティスが90項目にわたって詳しく解説されている名著ですよ。よりステップアップしたいストーンさんには必読の一冊です！
